...
import { 
  ...
  AlertTriangle, Search, Filter, ArrowUpDown 
} from 'lucide-preact';

export function AppTable({ apps }) {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterType, setFilterType] = useState('all');
  const [sortType, setSortType] = useState('newest'); // newest, oldest, name
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
          ...
          <div className="relative flex-1 sm:w-40">
            ...
            <select 
              ...
              className="w-full pl-9 pr-8 py-2 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-md text-sm appearance-none focus:outline-none cursor-pointer hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors"
            >
            ...
            {processedApps.length > 0 ? (
              processedApps.map((app) => (
                <tr key={app.name} className="group hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30 transition-colors">
                  <td className="px-6 py-4 font-bold text-black dark:text-white">{app.name}</td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      {app.security && (
                        <span className="flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border border-red-200 dark:border-red-800" title="Security Update">
                          <ShieldAlert size={12} /> Security
                        </span>
                      )}
                      {app.breaking && (
                        <span className="flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-200 dark:border-amber-800" title="Breaking Changes">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-3">
                      <a href={app.metadata?.homepage} target="_blank" rel="noopener noreferrer" className="p-1.5 text-zinc-400 hover:text-black dark:hover:text-white transition-colors" title="Homepage">
                        <ExternalLink size={16} />
                      </a>
                      <a href={app.release_notes} target="_blank" rel="noopener noreferrer" className="p-1.5 text-zinc-400 hover:text-black dark:hover:text-white transition-colors" title="Release Notes">
                        <ScrollText size={16} />
                      </a>
                    </div>
                  </td>
              ...
              <tr>
                <td colSpan={4} className="px-6 py-16 text-center">
                  ...
                  <button 
                    ...
                    className="mt-2 text-xs text-zinc-500 underline hover:text-black dark:hover:text-white"
                  >
                    Clear all filters
                  </button>
                </td>
